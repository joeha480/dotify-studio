		...
		switch (scope) {
			...
			case VOLUME: {
				...
				try {
					...
					} else if (vr.getFrom()>book.getVolumes()) {
						throw new FailedToGetRangeException(Messages.ERROR_VOLUME_NUMBER_OUT_OF_RANGE.localize(vr.getFrom(), book.getVolumes()));
					} else {
						return new Range(book.getFirstPage(vr.getFrom()), book.getLastPage(Math.min(vr.getTo(), book.getVolumes())));
					}
